...
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('bodcasts-fakar.index') }}">{{ __('global.bodcast.our_episodes') }}</a></li>
    <li class="breadcrumb-item">@yield('title')</li>
@endsection

@section('content')
<form action="{{ route('bodcasts-fakar.update', $row->id)}}" method="post">@csrf {{ method_field('PUT') }}
    <div class="row">
                    ...
                    <div class="tab-content">
                        @foreach($row->translations as $val)
                            <div class="tab-pane {{ ($val->locale==appLangKey())? 'active' : '' }}" id="tab-{{$val->locale}}">
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.title') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
