...
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ url('/admin/etmam-categories') }}">{{ __('global.etmam_category') }}</a></li>
    <li class="breadcrumb-item">@yield('title')</li>
@endsection

@section('content')
<form action="{{ url('/admin/etmam-categories/'.$row->id) }}" method="post">@csrf {{ method_field('PUT') }}
    <div class="row">
                    ...
                    <div class="tab-content">
                        ...
                        @foreach($row->translations as $val)
                            <div class="tab-pane {{ ($val->locale==appLangKey())? 'active' : '' }}" id="tab-{{$val->locale}}">
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.activity.categor_name') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
