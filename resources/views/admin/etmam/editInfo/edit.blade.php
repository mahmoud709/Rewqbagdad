    ...
    <form action="{{ url()->current() }}" method="post">@csrf
        <div class="row">
            ...
            <div class="col-md-4 mb-3">
                ...
                <input type="email" required name="contact_email" value="{{ old('contact_email', $row->contact_email) }}"
                    class="form-control" />
            </div>
            ...
            <div class="col-md-5 mb-3">
                ...
                <div class="input-group">
                    ...
                    <input id="thumbnail" required dir="ltr" class="form-control text-left" type="text"
                        name="img" value="{{ old('img', $row->img) }}" />
                </div>
            ...
            <div class="col-md-3 mb-3">
                ...
                <div class="input-group">
                    <span class="input-group-btn">
                        <a id="img2" data-input="thumimg2" data-preview="holderimg2"
                            class="btn btn-primary text-white">
                            <i class="fas fa-image"></i> {{ __('global.choose') }}
                        </a>
                    </span>
                    <input id="thumimg2" name="project_manager_photo" required dir="ltr" class="form-control"
                        type="text" name="news_img"
                        value="{{ old('project_manager_photo', $row->proejct_manager_img) }}" />
                </div>
                <div id="holderimg2" style="margin-top:15px;max-height:100px;">

                    @if (!empty(old('project_manager_photo', $row->proejct_manager_img)))
                        <img src="{{ old('project_manager_photo', $row->proejct_manager_img ?? '') }}"
                            style="height: 5rem;">
                    @endif
                </div>
            </div>
            ...
            <div class="col-md-4 mb-3">
                ...
                <input type="text" dir="ltr" class="form-control" name="whatsapp" value="{{ $row->whatsapp }}"
                    placeholder="https://wa.me/+201021464469" />
            </div>
                            ...
                            @foreach ($row->translations as $val)
                                <div class="tab-pane {{ $val->locale == appLangKey() ? 'active' : '' }}"
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.project_manager_name') }} ({{ LangNative($val->locale) }})
                                            <strong class="text-danger">*</strong></label>
                                        <input type="text" required name="project_manager[{{ $val->locale }}]"
                                            value="{{ old('project_manager')[$val->locale] ?? ($val->project_manager ?? '') }}"
                                            class="form-control" />
                                    </div>
                                    <div class="mb-3">
                                        <label>{{ __('global.rewaq.description') }} ({{ LangNative($val->locale) }})
                                            <strong class="text-danger">*</strong></label>
                                        <textarea dir="{{ langDir($val->locale) }}" name="content[{{ $val->locale }}]" class="form-control my-editor"
                                            rows="5">{!! old('content')[$val->locale] ?? $val->content !!}</textarea>
                                    </div>
