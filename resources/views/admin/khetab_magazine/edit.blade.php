...
@section('content')
    <form action="{{ url()->current() }}" method="post">@csrf
        <div class="row">
            ...
            @php
                $team_ids = json_decode($row->team_ids);
            @endphp
            @foreach ($teams as $team)
                <div class="col-md-3 mb-3">
                    <label> {{ $team->translation->job_title }}<strong class="text-danger">*</strong></label>
                    <select name="team_ids[]" class="form-control">


                        <option></option>
                        <option @if (!empty($team_ids) && in_array($team->id, $team_ids)) selected @endif value="{{ $team->id }}">
                            {{ $team->translation->name }}</option>
                    </select>
                </div>
            @endforeach
                                ...
                                <div class="tab-pane {{ $val->locale == appLangKey() ? 'active' : '' }}"
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-8 mb-3">
                                            <label>{{ __('global.khetab_magazine.description') }}
