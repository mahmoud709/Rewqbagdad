...
@section('title', __('global.khetab_magazine.team.add'))

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ url('/admin/khetab-magazine-team') }}">{{ __('global.khetab_magazine.team.title') }}</a></li>
    <li class="breadcrumb-item">@yield('title')</li>
@endsection

@section('content')
<form action="{{ url('/admin/khetab-magazine-team') }}" method="post">@csrf
    <div class="row">
        ...
        <div class="col-md-4 mb-3">
            <label>{{ __('global.khetab_magazine.team.email') }} <strong class="text-danger">*</strong></label>
            <input type="email" required name="email" value="{{old('email')}}" class="form-control" />
        </div>
        
        <div class="col-md-3 mb-3">
            <label>{{ __('global.khetab_magazine.team.cv_link') }} </label>
            <input type="url" name="cv_link" value="{{old('cv_link')}}" class="form-control" />
        </div>

        <div class="col-md-3 mb-3">
            <label>{{ __('global.khetab_magazine.team.img') }} <strong class="text-danger">*</strong></label>
        ...
        <div class="col-md-4 mb-3">
            <label>{{ __('global.khetab_magazine.team.job_title') }} <strong class="text-danger">*</strong></label>
            <select name="job_title" required class="form-control">
                @foreach (__('global.khetab_magazine.team.jobs_titles')[appLangKey()] as $key => $job)
                            ...
                            <div class="tab-pane {{ ($localeCode==appLangKey())? 'active' : '' }}" id="tab-{{$localeCode}}">
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.khetab_magazine.team.name') }} ({{ $properties['native'] }}) <strong class="text-danger">*</strong></label>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.khetab_magazine.team.desc') }} ({{ $properties['native'] }}) <strong class="text-danger">*</strong></label>
