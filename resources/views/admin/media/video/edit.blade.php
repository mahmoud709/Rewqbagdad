...
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ url('/admin/media-videos') }}">{{ __('global.media.video.title') }}</a></li>
    <li class="breadcrumb-item">@yield('title')</li>
@endsection

@section('content')
<form action="{{ url('/admin/media-videos/'.$row->id) }}" method="post">@csrf {{ method_field('PUT') }}
    <div class="row">
        
        <div class="col-md-6 mb-3">
            <label>{{ __('global.media.video.thum') }} <strong class="text-danger">*</strong></label>
        ...
        <div class="col-md-6 mb-3">
            <label>{{ __('global.media.video.video_url') }} <strong class="text-danger">*</strong></label>
            <input required dir="ltr" class="form-control" type="url" name="video_url" value="{{ old('video_url',$row->video_url) }}" />
        </div>
        
        <div class="col-md-4 mb-3">
            <label>{{ __('global.media.video.categor_name') }} <strong class="text-danger">*</strong></label>
            <select name="category_id" class="form-control">
                @foreach ($cats as $cat)
                    <option @if($cat->id==$row->category_id) selected @endif value="{{$cat->id}}">{{$cat->translation->name}}</option>
                @endforeach
            </select>
        </div>
                    ...
                    <div class="tab-content">
                        @foreach($row->translations as $val)
                            <div class="tab-pane {{ ($val->locale==appLangKey())? 'active' : '' }}" id="tab-{{$val->locale}}">
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.media.video.video_name') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
