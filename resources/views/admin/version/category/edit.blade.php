...
@section('content')
<form action="{{ url('/admin/version-categories/'.$row->id) }}" method="post">@csrf {{ method_field('PUT') }}
    <div class="row">
                    ...
                    <div class="tab-content">
                        ...
                        @foreach($row->translations as $val)
                            <div class="tab-pane {{ ($val->locale==appLangKey())? 'active' : '' }}" id="tab-{{$val->locale}}">
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label>{{ __('global.version.categor_name') }} ({{ LangNative($val->locale) }}) <strong class="text-danger">*</strong></label>
